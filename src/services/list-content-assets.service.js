...
exports.listContentAssets = async (id, args) => {
    try {
        if (!id) {
        ...
        const contentAssets = await db.asset.findAndCountAll(query);

        return urlUtils.formatListResponse(contentAssets, args.endpoint, args);
    } catch (err) {
        throw err;
    }
};
